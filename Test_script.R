...
tidycensus::census_api_key("CENSUS_API_KEY", install = TRUE) # See Sys.getenv("CENSUS_API_KEY")
...
vars_census <- c("P001001", "P012006", "P012007", "P012008", "P012009"
                 , "P012010", "P012011", "P012012", "P005003", "P005004"
                 , "P005006", "P005010", "P015001", "P005005", "P005007"
                 , "P005008", "P005009")

chi_sf1 <- tidycensus::get_decennial(geography = "tract", state = "IL",
                                     county = "Cook County"
...
chi_acs <- tidycensus::get_acs(geography = "tract", state = "IL",
                               county = "Cook County", variables = vars_acs,
                               year = 2011, output = "wide")

# Wrangle the data ############################################################

# herfindahl or fractionalization index
f_index <- function(x) 1 - rowSums(xˆ2)
# factor analysis
...
legend(-87.87, 41.73, legend = c("0.0", "0.25", "0.5", "0.75", "1.0"),
